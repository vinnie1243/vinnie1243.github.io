function make() {
    ...
    if(local == "true") {
        ...
        window.sessionStorage.setItem("difficulty", "braindead")
        document.getElementById("braindead").checked = true
    }    
    ...
    window.sessionStorage.setItem("aiStarted", false)
    ...
    for(var j = 0; j < 8; j++) {
        for(var i = 0; i < 8; i++) {
            ...
            e.addEventListener("click", (e) => {
                window.sessionStorage.setItem("hold", JSON.stringify(e))
            })
...
function clic(e) {
    ...
    var arr = gen(el.src, e.target.parentElement.id, color, false)
}
...
async function move(e, aiMoving) { 
    var move = window.sessionStorage.getItem("move")
    var selectedPiece = window.sessionStorage.getItem("sp")
    var sp = document.getElementById(selectedPiece)
    if(sp.classList.contains("white") == true && move == "b" && aiMoving == false) {
        return false
    } else if(sp.classList.contains("black") == true && move == "w" && aiMoving == false) {
        return false
    }
    console.log("Test")
    ...
    //stuff that actually moves the piece
    if(aiMoving == true) {
        var ele = window.sessionStorage.getItem("aimove")
        ele = document.getElementById(ele)
    } else {
        var ele = e.target.parentElement
    }
    var c = 0
    while (c == 0 && aiMoving == false) {
...
function setDiff(e) {
    ...
    window.sessionStorage.setItem("difficulty", diff)
    console.log(diff)
}